...
pub fn get_pdf_info_before_signing(file_path: String) -> Result<u32, Box<dyn std::error::Error>> {
    match get_page_count(file_path) {
        Ok(count) => Ok(count),
...
pub fn sign_pdf(file_path: String, signature_text: String) -> Result<(), Box<dyn std::error::Error>> {
    // Load the PDF document, propagating any errors.
    let mut document = lopdf::Document::load(&file_path).map_err(|e| {
        Box::new(std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to load PDF: {}", e))) as Box<dyn std::error::Error>
    })?;
    ...
    signature_dict.set("SubFilter", lopdf::Object::Name(b"adbe.pkcs7.detached".to_vec()));
    ...
    signature_dict.set("Contents", lopdf::Object::String(signature_text.into_bytes(), lopdf::StringFormat::Hexadecimal));
    ...
    fields.set("T", lopdf::Object::String(b"Signature1".to_vec(), lopdf::StringFormat::Literal)); // Field Name
    ...
    acroform.set("Fields", lopdf::Object::Array(vec![lopdf::Object::Reference(field_id)]));
    ...
    document.save(&file_path).map_err(|e| {
        Box::new(std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to save PDF: {}", e))) as Box<dyn std::error::Error>
    })?;
